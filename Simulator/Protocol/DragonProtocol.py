...
class DragonProtocol(CacheProtocol):
    ...
    def process_msg(self, msg, flags, caches):
        ...
        if not flags[msg.sender_id]:  # Start from Invalid State
            if msg.type == MessageType.WRITE_REQ:
                if cnt == 0:
                    ...
                    self.cpu_stats[CPU_NUMS]["update"] += 1
                    self.cpu_stats[CPU_NUMS]["data_traffic"] += (
                        CACHE_CONFIG.block_size + 4 * cnt
                    )
                    return CACHE_CONFIG.block_size // 2 + TIME_CONFIG.bus_update
            else:  # READ_REQ
                if cnt == 0:
            ...
            if caches[msg.sender_id].get_state() == StateHandler.exclusive():
                ...
                if cnt == 1:
                    ...
                    self.cpu_stats[CPU_NUMS]["update"] += 1
                    self.cpu_stats[CPU_NUMS]["data_traffic"] += 4 * (cnt - 1)
                    return TIME_CONFIG.bus_update
        ...
        return TIME_CONFIG.load_block_from_mem
